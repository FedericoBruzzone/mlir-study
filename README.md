...
> **Note on N=128 measurements:** each benchmark binary runs the kernel `NITER` times in a loop (N=128 → NITER=100) and reports wall-time ÷ NITER, amortising the ~50ms fixed process-startup cost to < 25% of the reported figure.
...
| Peak AMX FP32 | ~313 GFLOP/s (measured via Accelerate at N=1024) |
...
- Accelerate/AMX: 313 GFLOP/s (measured at N=1024)
...
- Softmax 512×512 — from `rq4_workloads.csv` *(AI=0.63, above ridge point ~0.53 — compute-bound but 4.7% efficiency due to scalar exp)*
- Conv2d 56×56 — from `rq4_workloads.csv`

> **N=128 points** use NITER=100 iterations per binary invocation; reported time is wall-time ÷ 100, so process-startup overhead is amortised to < 25%.
...
| Variant | IPC | Meaning |
|---------|-----|---------|
| affine no-tile | 1.76 | Baseline scalar loops |
| affine tile T=16 | 1.81 | Same code quality as no-tile |
| affine tile T=64 | 1.81 | **Identical assembly** to T=16 |
| vectorized T=16 | 1.98 | +~9% IPC from explicit NEON |

T=16 and T=64 generate **identical assembly with identical IPC** — yet at runtime T=64 is up to **~7× slower** than T=32 with `--O3 --mattr=apple-m4`. This discrepancy is explained by **cache set aliasing**: with apple-m4 NEON enabled, the stride between tiles at T=64 is an exact power-of-2 multiple of L1 size (64²×3×4B × N = multiple of 64KB), causing systematic eviction of tiles from the same cache sets.
